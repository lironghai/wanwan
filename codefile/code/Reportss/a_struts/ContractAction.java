package com.sinosoft.ems.contract;
...
public class ContractAction extends PermissionAction {
	...
	public String query() {
		...
		Specification<Application> spec = contract == null ? null : (new Specification<Application>() {
			@Override
			public Predicate toPredicate(Root<Application> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
				...
				if (!StringHelper.isEmpty(contract.getId())) sq.ilike("code", contract.getId());
	...
	public Application getContract() {
		return contract;
	}

	public void setContract(Application contract) {
