...
public class PaymentAction extends PermissionAction {
	...
	public String query() {
		...
		Specification<Application> spec = payment == null ? null : (new Specification<Application>() {
			@Override
			public Predicate toPredicate(Root<Application> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
				...
				if (!StringHelper.isEmpty(payment.getId())) sq.ilike("code", payment.getId());
	...
	/** 当前要处理的人员 */
	protected Application payment;

	public Application getPayment() {
		return payment;
	}

	public void setPayment(Application payment) {
