...
function factory(angular, $) {
	var namespace = "/setting/budgetSetting";
	/** 定义的service，会被注册为BudgetSettingService */
	var service = function($rootScope, $http, $q, $timeout) {
		...
		return {
			 ...
			 */
			save: function(budgetSetting, ns) {
				var deferred = $q.defer();
				var $this = this;
				$http.post(this.url("/save.do", ns), { entity: budgetSetting }).success(function(budgetSetting) {
					$this.put(budgetSetting);
					deferred.resolve(budgetSetting);
				});
				return deferred.promise;
			},
	...
	service.$name = "BudgetSettingService"; // 可以省略，系统会默认以"模块名+Service"进行注册.如果此模块可能被其他模块依赖,则不能省略

	/** 定义的controller，会被注册为BudgetSettingController */
	var controller = function($scope, $rootScope, $window, BudgetSettingService, LocaleService) {
		...
		$.extend($scope, {
			...
			query: function(params, pageRequest) {
				...
				params = $.extend(params || { }, BudgetSettingService.getCurrentParams() || { });
				BudgetSettingService.query(params, pageRequest, session.currentMenu.namespace).then(function(page) {
					current.page = page;
				});
			...
			add: function() {
				...
				current.entity = $.extend({ isNew: function() { return true; } }, BudgetSettingService.getCurrentParams());
				current.operator = "add";
			},
			
			/** 查看 */
			view: function(budgetSetting) {
				current.operator = "view";
				current.entity = budgetSetting;
			},

			/** 编辑 */
			edit: function(budgetSetting) {
				current.operator = "edit";
				if (!budgetSetting) budgetSetting = current.entity;
				current.entity = $.extend({ }, budgetSetting);
			},
			...
			/** 保存数据 */
			save: function(budgetSetting) {
				if (!current.operator) return;
				BudgetSettingService.save(budgetSetting || current.entity, session.currentMenu.namespace).then(function(entity) {
			 ...
			 */
			remove: function(budgetSetting) {
				if (!$window.confirm(LocaleService.getText("app.confirm.delete"))) return;
				if (!budgetSetting) budgetSetting = [ current.entity ];
				if (!angular.isArray(budgetSetting)) budgetSetting = [ budgetSetting ];
				var ids = [ ];
				
				var message = "";
				angular.forEach(budgetSetting, function(budgetSetting) {
					ids.push(budgetSetting.id);
				});
				if (ids.length < 1){
				...
				BudgetSettingService.remove(ids, session.currentMenu.namespace).then(function(budgetSetting) {
	...
	controller.$inject = [ "$scope", "$rootScope", "$window", "BudgetSettingService", "LocaleService"];
	controller.$name = "BudgetSettingController"; // 可以省略，系统会默认以"模块名+Controller"进行注册
...
define("BudgetSetting", [ "angular", "jquery" ], function(angular, $) {
