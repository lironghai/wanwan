...
function factory(angular, $) {
	var namespace = "/setting/application";
	/** 定义的service，会被注册为ApplicationService */
	var service = function($rootScope, $http, $q, $timeout) {
		...
		return {
			 ...
			 */
			save: function(application, ns) {
				var deferred = $q.defer();
				var $this = this;
				$http.post(this.url("/save.do", ns), { entity: application }).success(function(application) {
	...
	service.$name = "ApplicationService"; // 可以省略，系统会默认以"模块名+Service"进行注册.如果此模块可能被其他模块依赖,则不能省略

	/** 定义的controller，会被注册为ApplicationController */
	var controller = function($scope, $rootScope, $window, ApplicationService, LocaleService) {
		...
		$.extend($scope, {
			...
			query: function(params, pageRequest) {
				...
				params = $.extend(params || { }, ApplicationService.getCurrentParams() || { });
				ApplicationService.query(params, pageRequest, session.currentMenu.namespace).then(function(page) {
					current.page = page;
				});
			...
			add: function() {
				...
				current.entity = $.extend({ isNew: function() { return true; } }, ApplicationService.getCurrentParams());
				current.operator = "add";
			},
			
			/** 查看 */
			view: function(application) {
				current.operator = "view";
				current.entity = application;
			},

			/** 编辑 */
			edit: function(application) {
				current.operator = "edit";
				if (!application) application = current.entity;
				current.entity = $.extend({ }, application);
			},
			...
			save: function(application) {
				...
				ApplicationService.save(application || current.entity, session.currentMenu.namespace).then(function(entity) {
			...
			remove: function(application) {
				...
				if (!application) application = [ current.entity ];
				if (!angular.isArray(application)) application = [ application ];
				var ids = [ ];
				...
				ApplicationService.remove(ids, session.currentMenu.namespace).then(function(application) {
	...
	controller.$inject = [ "$scope", "$rootScope", "$window", "ApplicationService", "LocaleService"];
	controller.$name = "ApplicationController"; // 可以省略，系统会默认以"模块名+Controller"进行注册
...
define("Application", [ "angular", "jquery" ], function(angular, $) {
