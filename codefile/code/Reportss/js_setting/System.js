...
function factory(angular, $) {
	var namespace = "/setting/system";
	/** 定义的service，会被注册为SystemService */
	var service = function($rootScope, $http, $q, $timeout) {
		...
		return {
			 ...
			 */
			save: function(system, ns) {
				var deferred = $q.defer();
				var $this = this;
				$http.post(this.url("/save.do", ns), { entity: system }).success(function(system) {
					$this.put(system);
	...
	service.$name = "SystemService"; // 可以省略，系统会默认以"模块名+Service"进行注册.如果此模块可能被其他模块依赖,则不能省略

	/** 定义的controller，会被注册为SystemController */
	var controller = function($scope, $rootScope, $window, SystemService, LocaleService) {
		...
		$.extend($scope, {
			...
			query: function(params, pageRequest) {
				...
				params = $.extend(params || { }, SystemService.getCurrentParams() || { });
				SystemService.query(params, pageRequest, session.currentMenu.namespace).then(function(page) {
					current.page = page;
				});
			...
			add: function() {
				...
				current.entity = $.extend({ isNew: function() { return true; } }, SystemService.getCurrentParams());
				current.operator = "add";
			},
			
			/** 查看 */
			view: function(system) {
			...
			edit: function(system) {
				...
				if (!system) system = current.entity;
				current.entity = $.extend({ }, system);
			},
			...
			save: function(system) {
				...
				SystemService.save(system || current.entity, session.currentMenu.namespace).then(function(entity) {
			...
			remove: function(system) {
				...
				if (!system) system = [ current.entity ];
				if (!angular.isArray(system)) system = [ system ];
				var ids = [ ];
				
				var message = "";
				angular.forEach(system, function(system) {
					ids.push(system.id);
				});
				if (ids.length < 1){
				...
				SystemService.remove(ids, session.currentMenu.namespace).then(function(system) {
	...
	controller.$inject = [ "$scope", "$rootScope", "$window", "SystemService", "LocaleService"];
	controller.$name = "SystemController"; // 可以省略，系统会默认以"模块名+Controller"进行注册
...
define("System", [ "angular", "jquery" ], function(angular, $) {
