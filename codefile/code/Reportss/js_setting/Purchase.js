...
function factory(angular, $) {
	var namespace = "/setting/purchase";
	/** 定义的service，会被注册为PurchaseService */
	var service = function($rootScope, $http, $q, $timeout) {
		...
		return {
			 ...
			 */
			save: function(purchase, ns) {
				var deferred = $q.defer();
				var $this = this;
				$http.post(this.url("/save.do", ns), { entity: purchase }).success(function(purchase) {
					$this.put(purchase);
	...
	service.$name = "PurchaseService"; // 可以省略，系统会默认以"模块名+Service"进行注册.如果此模块可能被其他模块依赖,则不能省略

	/** 定义的controller，会被注册为PurchaseController */
	var controller = function($scope, $rootScope, $window, PurchaseService, LocaleService) {
		...
		$.extend($scope, {
			...
			query: function(params, pageRequest) {
				...
				params = $.extend(params || { }, PurchaseService.getCurrentParams() || { });
				PurchaseService.query(params, pageRequest, session.currentMenu.namespace).then(function(page) {
					current.page = page;
				});
			...
			add: function() {
				...
				current.entity = $.extend({ isNew: function() { return true; } }, PurchaseService.getCurrentParams());
			...
			view: function(purchase) {
				...
				current.entity = purchase;
			},

			/** 编辑 */
			edit: function(purchase) {
				current.operator = "edit";
				if (!purchase) purchase = current.entity;
				current.entity = $.extend({ }, purchase);
			},
			...
			save: function(purchase) {
				...
				PurchaseService.save(purchase || current.entity, session.currentMenu.namespace).then(function(entity) {
			...
			remove: function(purchase) {
				...
				if (!purchase) purchase = [ current.entity ];
				if (!angular.isArray(purchase)) purchase = [ purchase ];
				var ids = [ ];
				
				var message = "";
				angular.forEach(purchase, function(purchase) {
				...
				PurchaseService.remove(ids, session.currentMenu.namespace).then(function(purchase) {
	...
	controller.$inject = [ "$scope", "$rootScope", "$window", "PurchaseService", "LocaleService"];
	controller.$name = "PurchaseController"; // 可以省略，系统会默认以"模块名+Controller"进行注册
...
define("Purchase", [ "angular", "jquery" ], function(angular, $) {
