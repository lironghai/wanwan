...
function factory(angular, $) {
	var namespace = "/setting/purchaseSetting";
	/** 定义的service，会被注册为PurchaseSettingService */
	var service = function($rootScope, $http, $q, $timeout) {
		...
		return {
			 ...
			 */
			save: function(purchaseSetting, ns) {
				var deferred = $q.defer();
				var $this = this;
				$http.post(this.url("/save.do", ns), { entity: purchaseSetting }).success(function(purchaseSetting) {
					$this.put(purchaseSetting);
					deferred.resolve(purchaseSetting);
				});
				return deferred.promise;
			},
	...
	service.$name = "PurchaseSettingService"; // 可以省略，系统会默认以"模块名+Service"进行注册.如果此模块可能被其他模块依赖,则不能省略

	/** 定义的controller，会被注册为PurchaseSettingController */
	var controller = function($scope, $rootScope, $window, PurchaseSettingService, LocaleService) {
		...
		$.extend($scope, {
			...
			query: function(params, pageRequest) {
				...
				params = $.extend(params || { }, PurchaseSettingService.getCurrentParams() || { });
				PurchaseSettingService.query(params, pageRequest, session.currentMenu.namespace).then(function(page) {
					current.page = page;
				});
			...
			add: function() {
				...
				current.entity = $.extend({ isNew: function() { return true; } }, PurchaseSettingService.getCurrentParams());
				current.operator = "add";
			},
			
			/** 查看 */
			view: function(purchaseSetting) {
				current.operator = "view";
				current.entity = purchaseSetting;
			},

			/** 编辑 */
			edit: function(purchaseSetting) {
				current.operator = "edit";
				if (!purchaseSetting) purchaseSetting = current.entity;
				current.entity = $.extend({ }, purchaseSetting);
			},
			...
			/** 保存数据 */
			save: function(purchaseSetting) {
				if (!current.operator) return;
				PurchaseSettingService.save(purchaseSetting || current.entity, session.currentMenu.namespace).then(function(entity) {
			 ...
			 */
			remove: function(purchaseSetting) {
				if (!$window.confirm(LocaleService.getText("app.confirm.delete"))) return;
				if (!purchaseSetting) purchaseSetting = [ current.entity ];
				if (!angular.isArray(purchaseSetting)) purchaseSetting = [ purchaseSetting ];
				var ids = [ ];
				
				var message = "";
				angular.forEach(purchaseSetting, function(purchaseSetting) {
					ids.push(purchaseSetting.id);
				});
				if (ids.length < 1){
				...
				PurchaseSettingService.remove(ids, session.currentMenu.namespace).then(function(purchaseSetting) {
	...
	controller.$inject = [ "$scope", "$rootScope", "$window", "PurchaseSettingService", "LocaleService"];
	controller.$name = "PurchaseSettingController"; // 可以省略，系统会默认以"模块名+Controller"进行注册
...
define("PurchaseSetting", [ "angular", "jquery" ], function(angular, $) {
