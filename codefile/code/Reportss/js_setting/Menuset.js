...
function factory(angular, $) {
	var namespace = "/setting/menuset";
	/** 定义的service，会被注册为MenusetService */
	var service = function($rootScope, $http, $q, $timeout) {
		...
		return {
			 ...
			 */
			save: function(menuset, ns) {
				var deferred = $q.defer();
				var $this = this;
				$http.post(this.url("/save.do", ns), { entity: menuset }).success(function(menuset) {
					$this.put(menuset);
					deferred.resolve(menuset);
				});
				return deferred.promise;
			},
	...
	service.$name = "MenusetService"; // 可以省略，系统会默认以"模块名+Service"进行注册.如果此模块可能被其他模块依赖,则不能省略

	/** 定义的controller，会被注册为MenusetController */
	var controller = function($scope, $rootScope, $window, MenusetService, LocaleService) {
		...
		$.extend($scope, {
			...
			query: function(params, pageRequest) {
				...
				params = $.extend(params || { }, MenusetService.getCurrentParams() || { });
				MenusetService.query(params, pageRequest, session.currentMenu.namespace).then(function(page) {
					current.page = page;
				});
			...
			add: function() {
				...
				current.entity = $.extend({ isNew: function() { return true; } }, MenusetService.getCurrentParams());
				current.operator = "add";
			},
			
			/** 查看 */
			view: function(menuset) {
				current.operator = "view";
				current.entity = menuset;
			},

			/** 编辑 */
			edit: function(menuset) {
				current.operator = "edit";
				if (!menuset) menuset = current.entity;
				current.entity = $.extend({ }, menuset);
			},
			...
			/** 保存数据 */
			save: function(menuset) {
				if (!current.operator) return;
				MenusetService.save(menuset || current.entity, session.currentMenu.namespace).then(function(entity) {
			 ...
			 */
			remove: function(menuset) {
				if (!$window.confirm(LocaleService.getText("app.confirm.delete"))) return;
				if (!menuset) menuset = [ current.entity ];
				if (!angular.isArray(menuset)) menuset = [ menuset ];
				var ids = [ ];
				
				var message = "";
				angular.forEach(menuset, function(menuset) {
					ids.push(menuset.id);
				});
				if (ids.length < 1){
				...
				MenusetService.remove(ids, session.currentMenu.namespace).then(function(menuset) {
	...
	controller.$inject = [ "$scope", "$rootScope", "$window", "MenusetService", "LocaleService"];
	controller.$name = "MenusetController"; // 可以省略，系统会默认以"模块名+Controller"进行注册
...
define("Menuset", [ "angular", "jquery" ], function(angular, $) {
