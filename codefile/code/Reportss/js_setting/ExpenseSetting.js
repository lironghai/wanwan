...
function factory(angular, $) {
	var namespace = "/setting/expenseSetting";
	/** 定义的service，会被注册为ExpenseSettingService */
	var service = function($rootScope, $http, $q, $timeout) {
		...
		return {
			 ...
			 */
			save: function(expenseSetting, ns) {
				var deferred = $q.defer();
				var $this = this;
				$http.post(this.url("/save.do", ns), { entity: expenseSetting }).success(function(expenseSetting) {
					$this.put(expenseSetting);
					deferred.resolve(expenseSetting);
				});
				return deferred.promise;
			},
	...
	service.$name = "ExpenseSettingService"; // 可以省略，系统会默认以"模块名+Service"进行注册.如果此模块可能被其他模块依赖,则不能省略

	/** 定义的controller，会被注册为ExpenseSettingController */
	var controller = function($scope, $rootScope, $window, ExpenseSettingService, LocaleService) {
		...
		$.extend($scope, {
			...
			query: function(params, pageRequest) {
				...
				params = $.extend(params || { }, ExpenseSettingService.getCurrentParams() || { });
				ExpenseSettingService.query(params, pageRequest, session.currentMenu.namespace).then(function(page) {
					current.page = page;
				});
			...
			add: function() {
				...
				current.entity = $.extend({ isNew: function() { return true; } }, ExpenseSettingService.getCurrentParams());
				current.operator = "add";
			},
			
			/** 查看 */
			view: function(expenseSetting) {
				current.operator = "view";
				current.entity = expenseSetting;
			},

			/** 编辑 */
			edit: function(expenseSetting) {
				current.operator = "edit";
				if (!expenseSetting) expenseSetting = current.entity;
				current.entity = $.extend({ }, expenseSetting);
			},
			...
			/** 保存数据 */
			save: function(expenseSetting) {
				if (!current.operator) return;
				ExpenseSettingService.save(expenseSetting || current.entity, session.currentMenu.namespace).then(function(entity) {
			 ...
			 */
			remove: function(expenseSetting) {
				if (!$window.confirm(LocaleService.getText("app.confirm.delete"))) return;
				if (!expenseSetting) expenseSetting = [ current.entity ];
				if (!angular.isArray(expenseSetting)) expenseSetting = [ expenseSetting ];
				var ids = [ ];
				
				var message = "";
				angular.forEach(expenseSetting, function(expenseSetting) {
					ids.push(expenseSetting.id);
				});
				if (ids.length < 1){
				...
				ExpenseSettingService.remove(ids, session.currentMenu.namespace).then(function(expenseSetting) {
	...
	controller.$inject = [ "$scope", "$rootScope", "$window", "ExpenseSettingService", "LocaleService"];
	controller.$name = "ExpenseSettingController"; // 可以省略，系统会默认以"模块名+Controller"进行注册
...
define("ExpenseSetting", [ "angular", "jquery" ], function(angular, $) {
