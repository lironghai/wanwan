...
function factory(angular, $) {
	var namespace = "/setting/paymentSetting";
	/** 定义的service，会被注册为PaymentSettingService */
	var service = function($rootScope, $http, $q, $timeout) {
		...
		return {
			 ...
			 */
			save: function(paymentSetting, ns) {
				var deferred = $q.defer();
				var $this = this;
				$http.post(this.url("/save.do", ns), { entity: paymentSetting }).success(function(paymentSetting) {
					$this.put(paymentSetting);
					deferred.resolve(paymentSetting);
				});
				return deferred.promise;
			},
	...
	service.$name = "PaymentSettingService"; // 可以省略，系统会默认以"模块名+Service"进行注册.如果此模块可能被其他模块依赖,则不能省略

	/** 定义的controller，会被注册为PaymentSettingController */
	var controller = function($scope, $rootScope, $window, PaymentSettingService, LocaleService) {
		...
		$.extend($scope, {
			...
			query: function(params, pageRequest) {
				...
				params = $.extend(params || { }, PaymentSettingService.getCurrentParams() || { });
				PaymentSettingService.query(params, pageRequest, session.currentMenu.namespace).then(function(page) {
					current.page = page;
				});
			...
			add: function() {
				...
				current.entity = $.extend({ isNew: function() { return true; } }, PaymentSettingService.getCurrentParams());
				current.operator = "add";
			},
			
			/** 查看 */
			view: function(paymentSetting) {
				current.operator = "view";
				current.entity = paymentSetting;
			},

			/** 编辑 */
			edit: function(paymentSetting) {
				current.operator = "edit";
				if (!paymentSetting) paymentSetting = current.entity;
				current.entity = $.extend({ }, paymentSetting);
			},
			...
			/** 保存数据 */
			save: function(paymentSetting) {
				if (!current.operator) return;
				PaymentSettingService.save(paymentSetting || current.entity, session.currentMenu.namespace).then(function(entity) {
			 ...
			 */
			remove: function(paymentSetting) {
				if (!$window.confirm(LocaleService.getText("app.confirm.delete"))) return;
				if (!paymentSetting) paymentSetting = [ current.entity ];
				if (!angular.isArray(paymentSetting)) paymentSetting = [ paymentSetting ];
				var ids = [ ];
				
				var message = "";
				angular.forEach(paymentSetting, function(paymentSetting) {
					ids.push(paymentSetting.id);
				});
				if (ids.length < 1){
				...
				PaymentSettingService.remove(ids, session.currentMenu.namespace).then(function(paymentSetting) {
	...
	controller.$inject = [ "$scope", "$rootScope", "$window", "PaymentSettingService", "LocaleService"];
	controller.$name = "PaymentSettingController"; // 可以省略，系统会默认以"模块名+Controller"进行注册
...
define("PaymentSetting", [ "angular", "jquery" ], function(angular, $) {
