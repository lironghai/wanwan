...
function factory(angular, $) {
	var namespace = "/setting/common";
	/** 定义的service，会被注册为CommonService */
	var service = function($rootScope, $http, $q, $timeout) {
		...
		return {
			 ...
			 */
			save: function(common, ns) {
				var deferred = $q.defer();
				var $this = this;
				$http.post(this.url("/save.do", ns), { entity: common }).success(function(common) {
					$this.put(common);
	...
	service.$name = "CommonService"; // 可以省略，系统会默认以"模块名+Service"进行注册.如果此模块可能被其他模块依赖,则不能省略

	/** 定义的controller，会被注册为CommonController */
	var controller = function($scope, $rootScope, $window, CommonService, LocaleService) {
		...
		$.extend($scope, {
			...
			query: function(params, pageRequest) {
				...
				params = $.extend(params || { }, CommonService.getCurrentParams() || { });
				CommonService.query(params, pageRequest, session.currentMenu.namespace).then(function(page) {
					current.page = page;
				});
			...
			add: function() {
				...
				current.entity = $.extend({ isNew: function() { return true; } }, CommonService.getCurrentParams());
				current.operator = "add";
			},
			
			/** 查看 */
			view: function(common) {
				current.operator = "view";
				current.entity = common;
			},

			/** 编辑 */
			edit: function(common) {
				current.operator = "edit";
				if (!common) common = current.entity;
				current.entity = $.extend({ }, common);
			},
			...
			save: function(common) {
				...
				CommonService.save(common || current.entity, session.currentMenu.namespace).then(function(entity) {
			 ...
			 */
			remove: function(common) {
				if (!$window.confirm(LocaleService.getText("app.confirm.delete"))) return;
				if (!common) common = [ current.entity ];
				if (!angular.isArray(common)) common = [ common ];
				var ids = [ ];
				
				var message = "";
				angular.forEach(common, function(common) {
					ids.push(common.id);
				});
				if (ids.length < 1){
				...
				CommonService.remove(ids, session.currentMenu.namespace).then(function(common) {
	...
	controller.$inject = [ "$scope", "$rootScope", "$window", "CommonService", "LocaleService"];
	controller.$name = "CommonController"; // 可以省略，系统会默认以"模块名+Controller"进行注册
...
define("Common", [ "angular", "jquery" ], function(angular, $) {
