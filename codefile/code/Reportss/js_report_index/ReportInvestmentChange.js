(function(factory) {
	// 定义ReportInvestmentChange模块
	define("ReportInvestmentChange", [ "angular", "jquery" ], function(angular, $) {
		return factory(angular, $);
	});
})(function factory(angular, $) {	// 定义模块的函数
	"use strict";
	var namespace = "/report/investmentChange";	// 此模块的namespace
	var all = {};	// 所有部门,主键为id
	var allCodes = { };	// 主键为code的部门
	
	/** 定义的service，会被注册为ReportInvestmentChangeService */
	...
	service.$name = "ReportInvestmentChangeService"; // 可以省略，系统会默认以"模块名+Service"进行注册.如果此模块可能被其他模块依赖,则不能省略

	/** 定义的controller，会被注册为ReportInvestmentChangeController */
	var controller = function($scope, $rootScope, $window, ReportInvestmentChangeService, LocaleService) {
		...
		$.extend($scope, {
			...
			query: function(params, pageRequest) {
				...
				ReportInvestmentChangeService.query(params, pageRequest).then(function(page) {
					current.page = page;
				});
	...
	controller.$inject = [ "$scope", "$rootScope", "$window", "ReportInvestmentChangeService", "LocaleService" ];
	controller.$name = "ReportInvestmentChangeController"; // 可以省略，系统会默认以"模块名+Service"进行注册.如果此模块可能被其他模块依赖,则不能省略
