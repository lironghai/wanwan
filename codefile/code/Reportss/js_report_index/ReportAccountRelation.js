(function(factory) {
	// 定义ReportAccountRelation模块
	define("ReportAccountRelation", [ "angular", "jquery" ], function(angular, $) {
		return factory(angular, $);
	});
})(function factory(angular, $) {	// 定义模块的函数
	"use strict";
	var namespace = "/report/accountRelation";	// 此模块的namespace
	var all = {};	// 所有部门,主键为id
	var allCodes = { };	// 主键为code的部门
	
	/** 定义的service，会被注册为ReportAccountRelationService */
	...
	service.$name = "ReportAccountRelationService"; // 可以省略，系统会默认以"模块名+Service"进行注册.如果此模块可能被其他模块依赖,则不能省略

	/** 定义的controller，会被注册为ReportAccountRelationController */
	var controller = function($scope, $rootScope, $window, ReportAccountRelationService, LocaleService) {
		...
		$.extend($scope, {
			...
			query: function(params, pageRequest) {
				...
				ReportAccountRelationService.query(params, pageRequest).then(function(page) {
					current.page = page;
				});
	...
	controller.$inject = [ "$scope", "$rootScope", "$window", "ReportAccountRelationService", "LocaleService" ];
	controller.$name = "ReportAccountRelationController"; // 可以省略，系统会默认以"模块名+Service"进行注册.如果此模块可能被其他模块依赖,则不能省略
