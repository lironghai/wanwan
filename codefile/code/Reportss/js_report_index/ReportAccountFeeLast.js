(function(factory) {
	// 定义ReportAccountFeeLast模块
	define("ReportAccountFeeLast", [ "angular", "jquery" ], function(angular, $) {
		return factory(angular, $);
	});
})(function factory(angular, $) {	// 定义模块的函数
	"use strict";
	var namespace = "/report/accountFeeLast";	// 此模块的namespace
	var all = {};	// 所有部门,主键为id
	var allCodes = { };	// 主键为code的部门
	
	/** 定义的service，会被注册为ReportAccountFeeLastService */
	...
	service.$name = "ReportAccountFeeLastService"; // 可以省略，系统会默认以"模块名+Service"进行注册.如果此模块可能被其他模块依赖,则不能省略

	/** 定义的controller，会被注册为ReportAccountFeeLastController */
	var controller = function($scope, $rootScope, $window, ReportAccountFeeLastService, LocaleService) {
		...
		$.extend($scope, {
			...
			query: function(params, pageRequest) {
				...
				ReportAccountFeeLastService.query(params, pageRequest).then(function(page) {
					current.page = page;
				});
	...
	controller.$inject = [ "$scope", "$rootScope", "$window", "ReportAccountFeeLastService", "LocaleService" ];
	controller.$name = "ReportAccountFeeLastController"; // 可以省略，系统会默认以"模块名+Service"进行注册.如果此模块可能被其他模块依赖,则不能省略
