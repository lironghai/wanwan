(function(factory) {
	// 定义ReportFoundRecord模块
	define("ReportFoundRecord", [ "angular", "jquery" ], function(angular, $) {
		return factory(angular, $);
	});
})(function factory(angular, $) {	// 定义模块的函数
	"use strict";
	var namespace = "/report/foundRecord";	// 此模块的namespace
	var all = {};	// 所有部门,主键为id
	var allCodes = { };	// 主键为code的部门
	
	/** 定义的service，会被注册为ReportFoundRecordService */
	...
	service.$name = "ReportFoundRecordService"; // 可以省略，系统会默认以"模块名+Service"进行注册.如果此模块可能被其他模块依赖,则不能省略

	/** 定义的controller，会被注册为ReportFoundRecordController */
	var controller = function($scope, $rootScope, $window, ReportFoundRecordService, LocaleService) {
		...
		$.extend($scope, {
			...
			query: function(params, pageRequest) {
				...
				ReportFoundRecordService.query(params, pageRequest).then(function(page) {
					current.page = page;
				});
	...
	controller.$inject = [ "$scope", "$rootScope", "$window", "ReportFoundRecordService", "LocaleService" ];
	controller.$name = "ReportFoundRecordController"; // 可以省略，系统会默认以"模块名+Service"进行注册.如果此模块可能被其他模块依赖,则不能省略
