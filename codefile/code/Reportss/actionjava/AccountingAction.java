...
public class AccountingAction extends PermissionAction {
	...
	public String query() {
		...
		Specification<Application> spec = accounting == null ? null : (new Specification<Application>() {
			@Override
			public Predicate toPredicate(Root<Application> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
				...
				if (!StringHelper.isEmpty(accounting.getId())) sq.ilike("code", accounting.getId());
	...
	/** 当前要处理的人员 */
	protected Application accounting;

	public Application getAccounting() {
		return accounting;
	}

	public void setAccounting(Application accounting) {
