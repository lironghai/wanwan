(function(factory) {
	// 定义Report@Child模块
	define("Report@Child", [ "angular", "jquery" ], function(angular, $) {
		return factory(angular, $);
	});
})(function factory(angular, $) {	// 定义模块的函数
	"use strict";
	var namespace = "/report/@child";	// 此模块的namespace
	var all = {};	// 所有部门,主键为id
	var allCodes = { };	// 主键为code的部门
	
	/** 定义的service，会被注册为Report@ChildService */
	...
	service.$name = "Report@ChildService"; // 可以省略，系统会默认以"模块名+Service"进行注册.如果此模块可能被其他模块依赖,则不能省略

	/** 定义的controller，会被注册为Report@ChildController */
	var controller = function($scope, $rootScope, $window, Report@ChildService, LocaleService) {
		...
		$.extend($scope, {
			...
			query: function(params, pageRequest) {
				...
				Report@ChildService.query(params, pageRequest).then(function(page) {
					current.page = page;
				});
	...
	controller.$inject = [ "$scope", "$rootScope", "$window", "Report@ChildService", "LocaleService" ];
	controller.$name = "Report@ChildController"; // 可以省略，系统会默认以"模块名+Service"进行注册.如果此模块可能被其他模块依赖,则不能省略
